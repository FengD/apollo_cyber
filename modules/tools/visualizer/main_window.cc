...
struct MainWindow::VideoImgProxy {
  ...
  union {
    CyberChannReader<crdc::airi::Image>* image_reader_;
    CyberChannReader<crdc::airi::CompressedImage>*
...
struct MainWindow::RadarData {
  ...
  CyberChannReader<crdc::airi::RadarObstacles>* channel_reader_;
  ...
  if (b) {
    ...
    if (!radarProxy->channel_reader_) {
      radarProxy->channel_reader_ =
          new CyberChannReader<crdc::airi::RadarObstacles>();
      ...
      auto radarcallback =
          [this, radarProxy](
              const std::shared_ptr<crdc::airi::RadarObstacles>& pdata) {
...
void MainWindow::RadarRenderCallback(
    const std::shared_ptr<const crdc::airi::RadarObstacles>& rawData,
    RadarData* radar) {
...
void MainWindow::PointCloudReaderCallback(
    const std::shared_ptr<crdc::airi::PointCloud>& pdata) {
  pointcloud_reader_mutex_.lock();
  pointcloud_reader_mutex_.unlock();
  ...
  if (b) {
    ...
    if (!pointcloud_channel_Reader_) {
      pointcloud_channel_Reader_ =
          new CyberChannReader<crdc::airi::PointCloud>();
      ...
      auto pointCallback =
          [this](const std::shared_ptr<crdc::airi::PointCloud>& pdata) {
...
void MainWindow::ImageReaderCallback(
    const std::shared_ptr<const crdc::airi::Image>& imgData,
...
void MainWindow::ImageReaderCallback(
    const std::shared_ptr<const crdc::airi::CompressedImage>& imgData,
    VideoImgProxy* theVideoImgProxy) {
  theVideoImgProxy->reader_mutex_.lock();
  if (theVideoImgProxy->dynamic_texture_ != nullptr && imgData != nullptr) {
    QImage img;
    ...
    if (!theVideoImg->image_reader_) {
      if (theVideoImg->isCompressedImage_) {
        theVideoImg->compressed_image_reader_ =
            new CyberChannReader<crdc::airi::CompressedImage>();
      } else {
        theVideoImg->image_reader_ =
            new CyberChannReader<crdc::airi::Image>();
      }
      ...
      if (theVideoImg->isCompressedImage_) {
        auto videoCallback =
            [this, theVideoImg](
                const std::shared_ptr<crdc::airi::CompressedImage>&
        ...
        auto videoCallback =
            [this, theVideoImg](
                const std::shared_ptr<crdc::airi::Image>& pdata) {
